...
def log2_transform_and_center(data_matrix):
    """Perform log2 transformation and median centering on the data matrix."""
    # Log2 transformation
    log2_data = np.log2(data_matrix + 1)  # Adding 1 to avoid log(0)

    # Median centering
    centered_data = log2_data - np.median(log2_data, axis=0)
...
parser.add_option("--fdr", dest="fdr",
                  default=0.001,
                  help="FDR threshold: default =< 0.001")


parser.add_option("-l", "--logfc", dest="out",
                  default=2.0,
                  help="logFC threshold: default => 2.0",
                  metavar="FILE")
...
logfile = "filter_RNAseqogFC_%.2f_FDR_%.3f.log" % (float(LOGFC), float(FDR))
# Run as script
