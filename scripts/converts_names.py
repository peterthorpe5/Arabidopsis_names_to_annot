...
def parse_gene_des():
    ...
    f_in = open(os.path.join(script_dir, 
                             "data", "gene_description_20131231.txt"), "r")
    for line in f_in:
        if test_line(line):
            data = line.split("\t")
            if len(data) < 3:
    ...
    for line in f_in:
        if test_line(line):
            ...
            if line.startswith("Row.names"):
                line = line.replace("Row.names", "gene\tgeneID\tgene_class")
            ...
            if "GLM.edgeR.DE" in infile:
                ...
                out_data = "%s\t%s\t%s\t%s" % (subject, gene_id, nbl_type, gene_custom_class)
...
if __name__ == '__main__':
    ...
    for dirpath, subdirs, files in os.walk(directory):
        for x in files:
            ...
            if x.endswith("subset") or x.endswith("FDR_0.001"):
