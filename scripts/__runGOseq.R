...
factor_labeling = read.table("vir1_low_vs_vir1_high.GLM.edgeR.DE_LOGFC_1.6_FDR_0.01.names", row.names=1)
...
# get gene lengths
gene_lengths = read.table("gene_len_GO.txt", header=T, row.names=1, com='')
gene_lengths = as.matrix(gene_lengths[,1,drop=F])


# get background gene list
background = read.table("At.gene.counts.min10.matrix", header=T, row.names=1)
...
GO_info = read.table("Go_final.txt", header=F, row.names=1,stringsAsFactors=F)
...
go_enrich_filename = paste("vir1_low_vs_vir1_high.GLM.edgeR.DE_LOGFC_1.6_FDR_0.01.names", '.GOseq.enriched', sep='')
    ...
    go_depleted_filename = paste("vir1_low_vs_vir1_high.GLM.edgeR.DE_LOGFC_1.6_FDR_0.01.names", '.GOseq.depleted', sep='')
